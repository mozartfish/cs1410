...
public class MethodLibrary
{
    ...
    public static String flip (String original, char pattern)
    {
        ...
        return original.substring(index+1) + original.substring(0, index);
    }
    ...
    {
        Scanner s = new Scanner(integerList);
        int largest = Math.max(s.nextInt(), s.nextInt());
        largest = Math.max(largest, s.nextInt());
        largest = Math.max(largest, s.nextInt());
        largest = Math.max(largest, s.nextInt());
        s.close();
        return largest;
     ...
     * is false.
     * 
     * The two parameters must be of the form YYYY-MM-DD where YYYY is a year, MM is the two-digit number of a month, DD
    ...
    {
        String d1 = date1.substring(7) + date1.substring(0, 6);
        String d2 = date2.substring(7) + date2.substring(0, 6);
    ...
    {
        BigInteger b1 = new BigInteger(integer1);
        BigInteger b2 = new BigInteger(integer2);
