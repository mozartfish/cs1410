 ...
 */
public class Asteroid extends Participant implements ShipDestroyer, BulletDestroyer, AlienShipDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        ...
        {
            Dust d = new Dust(getX(), getY(), controller);
            ...
            controller.asteroidDestroyed(this.size);
            
            if (p instanceof Ship || p instanceof Bullet || p instanceof XWing || p instanceof XWingLaser)
            {
                controller.increaseScore(ASTEROID_SCORE[this.size]);
            }
            ...
            try 
            {
                controller.addParticipant(new Asteroid(variety, size, this.getX(), this.getY(), RANDOM.nextInt(ASTEROID_SPEED[size]) + 1, controller));
                controller.addParticipant(new Asteroid(variety, size, this.getX(), this.getY(), RANDOM.nextInt(ASTEROID_SPEED[size]) + 1, controller));
            }
            catch (Exception e) {
            }
